...
class DrawBoard(Frame):
    ...
    def initUI(self, master=None):
        ...
        ImageLabel.grid(row=1, column=2, rowspan=10, columnspan=2,sticky='n')
        self.board = ImageLabel
        #ImageLabel.bind("<Button-1>", self.mousePressed)
        ImageLabel.bind("<B1-Motion>", self.leftMouseMoved)
        
        Hint = Label(master, bg='#FAD2C2', text=self.hint, anchor=N, width=20, height=10).grid(sticky='n', row = 1, column=4)
        Msg = Label(master, bg='#FAD2C2', text=self.chat, anchor=NW, width=20, height=10).grid(sticky='n', row = 2, column=4)
        Chat = Entry(master, width=20).grid(sticky='n', row = 3, column=4)
        self.timerFired()
