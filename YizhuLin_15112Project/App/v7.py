...
class DrawBoard(Frame):
    ...
    def __init__(self, master=None):
        ...
        threading.Thread(target=Client, args=(self,)).start()
        #self.connect()
        self.initUI(master)
    ...
    def changeColor0(self):
        if self.state == 'DRAW':self.color=self.colorList[0]
    def changeColor1(self):
        if self.state == 'DRAW':self.color=self.colorList[1]
    def changeColor2(self):
        if self.state == 'DRAW':self.color=self.colorList[2]
    def changeColor3(self):
        if self.state == 'DRAW':self.color=self.colorList[3]
    def changeColor4(self):
        if self.state == 'DRAW':self.color=self.colorList[4]
    def changeColor5(self):
        if self.state == 'DRAW':self.color=self.colorList[5]
    def changeColor6(self):
        if self.state == 'DRAW':self.color=self.colorList[6]
    def changeColor7(self):
        if self.state == 'DRAW':self.color=self.colorList[7]
    def changeColor8(self):
        if self.state == 'DRAW':self.color=self.colorList[8]
    def changeColor9(self):
        if self.state == 'DRAW':self.color=self.colorList[9]
        
    def changeBrushSize0(self):
        if self.state == 'DRAW':self.brushSize=5
    def changeBrushSize1(self):
        if self.state == 'DRAW':self.brushSize=10
    def changeBrushSize2(self):
        if self.state == 'DRAW':self.brushSize=20
